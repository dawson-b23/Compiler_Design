 ...
 void treeTraverseStmt(TreeNode *syntree, SymbolTable *symtab) {
    ...
    switch(syntree->kind.stmt) {
        ...
        case ReturnK:
            ...
            // figure out to set expected_type and expected_return from treenodes.h
            if(typeCheck->type != Void && c0 == NULL) {
                printf("SEMANTIC ERROR(%d): Function '%s' at line %d is expecting to return %s but return has no value.\n", syntree->lineno, typeCheck->attr.name, typeCheck->lineno, expTypeToStr(typeCheck->type));
                numErrors++;
            }
            printf("%s type: %d", typeCheck->attr.name, typeCheck->type);
            treeTraverse(c0, symtab);
            treeTraverse(c1, symtab);
            break;
